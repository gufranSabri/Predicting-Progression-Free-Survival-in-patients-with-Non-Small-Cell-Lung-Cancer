            ...
            for x, y in indices:
                ...
                if mask[x_mask, y_mask] == 1:
                    ...
                    if (mask_patch.sum() > BACKGROUND_THRESHOLD * mask_patch.size) and not (is_low_contrast(patch)):
                        ...
                        i += 1
                        indices_save.append((x,y))
                if i >= max_patches_per_slide:
                    break
            dset = f.create_dataset('num_patches', data=i)
            dset = f.create_dataset('locations', data=indices_save)
        if i == 0:
...
if __name__ == '__main__':
    ...
    if os.path.exists(args.patch_path):
        already_made = os.listdir(args.patch_path)
        print(f'Already made patches for {len(already_made)} slides')
        print(f'Original nr slides {len(slide_list)}')
        slide_list = [s for s in slide_list if s.replace('.svs', '') not in already_made]
        print(f'New nr slides {len(slide_list)}')
    else:
